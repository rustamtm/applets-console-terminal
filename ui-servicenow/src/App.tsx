...
const SN_SHORTCUT_HINTS: Record<SnowMenuActionId, string> = {
  newSession: "Ctrl/⌘+Alt+N",
  openGemini: "Ctrl/⌘+Alt+G",
  openCodex: "Ctrl/⌘+Alt+C",
  disconnect: "Ctrl/⌘+Alt+D",
  reconnect: "Ctrl/⌘+Alt+R",
  deleteSession: "Ctrl/⌘+Alt+X",
  sessions: "Ctrl/⌘+Alt+S",
  settings: "Ctrl/⌘+Alt+E",
  aiRename: "Ctrl/⌘+Alt+A",
  codexBrief: "Ctrl/⌘+Alt+B",
  openAllTmux: "Ctrl/⌘+Alt+O",
  refreshTab: "Ctrl/⌘+Alt+F",
  docs: "Ctrl/⌘+Alt+H",
  tasks: "Ctrl/⌘+Alt+L"
};

const SN_MENU_KEYMAP: Record<string, SnowMenuActionId> = {
  n: "newSession",
  g: "openGemini",
  c: "openCodex",
  d: "disconnect",
...
const SN_GLOBAL_KEYMAP: Record<string, SnowMenuActionId | "toggleMenu"> = {
  KeyM: "toggleMenu",
  KeyN: "newSession",
  KeyG: "openGemini",
  KeyC: "openCodex",
...
const SN_MENU_ICON_NAMES: Record<SnowMenuActionId, string> = {
  newSession: "ui-circle-plus.svg",
  openGemini: "ui-sparkles.svg",
  openCodex: "ui-codex-brief.svg",
  disconnect: "ui-square-stop.svg",
  reconnect: "ui-arrow-right-to-line.svg",
  deleteSession: "ui-trash-2.svg",
  sessions: "ui-layers.svg",
  settings: "ui-settings.svg",
  aiRename: "ui-sparkles.svg",
  codexBrief: "ui-codex-brief.svg",
  openAllTmux: "ui-open-all.svg",
  refreshTab: "ui-refresh-cw.svg",
  docs: "ui-folder.svg",
  tasks: "ui-clipboard.svg"
};
...
const TERMINAL_EVENT_OVERRIDES_STORAGE_KEY = "console.terminalEventOverrides.v1";
const QUICK_GEMINI_TMUX_NAME_STORAGE_KEY = "console.quickGeminiTmuxName.v1";
const QUICK_GEMINI_COMMAND_STORAGE_KEY = "console.quickGeminiCommand.v1";
const QUICK_CODEX_TMUX_NAME_STORAGE_KEY = "console.quickCodexTmuxName.v1";
const QUICK_CODEX_COMMAND_STORAGE_KEY = "console.quickCodexCommand.v1";
const DEFAULT_QUICK_GEMINI_TMUX_NAME = "gemini";
const DEFAULT_QUICK_GEMINI_COMMAND = "gemini";
const DEFAULT_QUICK_CODEX_TMUX_NAME = "codex";
const DEFAULT_QUICK_CODEX_COMMAND = "codex";
...
type PendingQuickLaunchCommand = {
  sessionId: string;
  command: string;
  label: "Gemini" | "Codex";
  tmuxName: string;
...
function defaultTerminalEventOverridesEnabled(): boolean {
  return true;
}

function loadTerminalEventOverridesEnabled(): boolean {
  if (typeof window === "undefined") return defaultTerminalEventOverridesEnabled();
  try {
    const raw = localStorage.getItem(TERMINAL_EVENT_OVERRIDES_STORAGE_KEY);
    if (raw === null) return defaultTerminalEventOverridesEnabled();
    return raw === "1";
  } catch {
    return defaultTerminalEventOverridesEnabled();
  }
}

function saveTerminalEventOverridesEnabled(enabled: boolean) {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(TERMINAL_EVENT_OVERRIDES_STORAGE_KEY, enabled ? "1" : "0");
  } catch {
    // ignore storage failures
  }
}
...
function loadQuickLaunchTmuxName(storageKey: string, fallback: string): string {
  if (typeof window === "undefined") return fallback;
  try {
    const raw = localStorage.getItem(storageKey);
    const trimmed = String(raw ?? "").trim();
    if (trimmed && isValidTmuxName(trimmed)) return trimmed;
  } catch {
    // ignore storage failures
  }
  return fallback;
...
function saveQuickLaunchTmuxName(storageKey: string, value: string, fallback: string) {
  if (typeof window === "undefined") return;
  const next = sanitizeTmuxNameCandidate(value) || fallback;
  try {
...
function loadQuickLaunchCommand(storageKey: string, fallback: string): string {
  if (typeof window === "undefined") return fallback;
  try {
    const raw = localStorage.getItem(storageKey);
    const trimmed = String(raw ?? "").trim();
...
function saveQuickLaunchCommand(storageKey: string, command: string, fallback: string) {
  if (typeof window === "undefined") return;
  const next = String(command ?? "").trim() || fallback;
  try {
    localStorage.setItem(storageKey, next);
  } catch {
    // ignore storage failures
...
const AI_AUTONAME_ON_CODEX_DONE_STORAGE_KEY = "console.aiAutoNameOnCodexDone.v1";
...
function preferCodeRootCwd(raw: string | null | undefined): string | undefined {
  const parsed = normalizeCwdCandidate(String(raw ?? ""));
  if (!parsed) return undefined;
  const trimmed = parsed.replace(/\/+$/, "");
  if (trimmed.endsWith("/Code/applets")) {
    return trimmed.slice(0, -"/applets".length);
  }
...
function loadAiAutoNameOnCodexDone(): boolean {
  if (typeof window === "undefined") return false;
  try {
    const raw = localStorage.getItem(AI_AUTONAME_ON_CODEX_DONE_STORAGE_KEY);
    if (raw === null) return true;
    return raw === "1";
  } catch {
    return true;
...
function saveAiAutoNameOnCodexDone(enabled: boolean) {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(AI_AUTONAME_ON_CODEX_DONE_STORAGE_KEY, enabled ? "1" : "0");
  } catch {
    // ignore storage failures
  }
}
...
function formatSessionFallbackSuffix(payload: { fallbackMode?: string; fallbackFrom?: string }): string {
  const mode = String(payload.fallbackMode || "").trim();
  if (!mode) return "";
  const from = String(payload.fallbackFrom || "").trim();
  if (from) return ` (fallback ${from}->${mode})`;
...
export function App() {
  ...
  const [terminalEventOverridesEnabled, setTerminalEventOverridesEnabled] = useState<boolean>(
    () => loadTerminalEventOverridesEnabled()
  );
  const [quickGeminiTmuxName, setQuickGeminiTmuxName] = useState<string>(() =>
    loadQuickLaunchTmuxName(QUICK_GEMINI_TMUX_NAME_STORAGE_KEY, DEFAULT_QUICK_GEMINI_TMUX_NAME)
  );
  const [quickGeminiCommand, setQuickGeminiCommand] = useState<string>(() =>
    loadQuickLaunchCommand(QUICK_GEMINI_COMMAND_STORAGE_KEY, DEFAULT_QUICK_GEMINI_COMMAND)
  );
  const [quickCodexTmuxName, setQuickCodexTmuxName] = useState<string>(() =>
    loadQuickLaunchTmuxName(QUICK_CODEX_TMUX_NAME_STORAGE_KEY, DEFAULT_QUICK_CODEX_TMUX_NAME)
  );
  const [quickCodexCommand, setQuickCodexCommand] = useState<string>(() =>
    loadQuickLaunchCommand(QUICK_CODEX_COMMAND_STORAGE_KEY, DEFAULT_QUICK_CODEX_COMMAND)
  );
  ...
  const [codexSandbox] = useState<"read-only" | "workspace-write" | "danger-full-access">("danger-full-access");
  ...
  const codexPromptUsedRef = useRef<string>("");
  const codexModelUsedRef = useRef<string>("");
  const currentKeyRef = useRef<string | null>(null);
  const pendingQuickLaunchCommandRef = useRef<PendingQuickLaunchCommand | null>(null);
  const suggestAiNameForSessionRef = useRef<null | ((targetSessionId: string, key: string, options?: any) => any)>(null);
  ...
  const [aiAutoNameOnCodexDone, setAiAutoNameOnCodexDone] = useState<boolean>(() => loadAiAutoNameOnCodexDone());
  ...
  const applyTerminalEventOverridesEnabled = useCallback((enabled: boolean) => {
    setTerminalEventOverridesEnabled(enabled);
    saveTerminalEventOverridesEnabled(enabled);
  }, []);

  const applyQuickGeminiTmuxName = useCallback((value: string) => {
    const next = sanitizeTmuxNameCandidate(value) || DEFAULT_QUICK_GEMINI_TMUX_NAME;
    setQuickGeminiTmuxName(next);
    saveQuickLaunchTmuxName(QUICK_GEMINI_TMUX_NAME_STORAGE_KEY, next, DEFAULT_QUICK_GEMINI_TMUX_NAME);
  }, []);

  const applyQuickGeminiCommand = useCallback((value: string) => {
    const next = String(value ?? "");
    setQuickGeminiCommand(next);
    saveQuickLaunchCommand(QUICK_GEMINI_COMMAND_STORAGE_KEY, next, DEFAULT_QUICK_GEMINI_COMMAND);
  }, []);

  const applyQuickCodexTmuxName = useCallback((value: string) => {
    const next = sanitizeTmuxNameCandidate(value) || DEFAULT_QUICK_CODEX_TMUX_NAME;
    setQuickCodexTmuxName(next);
    saveQuickLaunchTmuxName(QUICK_CODEX_TMUX_NAME_STORAGE_KEY, next, DEFAULT_QUICK_CODEX_TMUX_NAME);
  }, []);

  const applyQuickCodexCommand = useCallback((value: string) => {
    const next = String(value ?? "");
    setQuickCodexCommand(next);
    saveQuickLaunchCommand(QUICK_CODEX_COMMAND_STORAGE_KEY, next, DEFAULT_QUICK_CODEX_COMMAND);
  }, []);
  ...
  const applyAiAutoNameOnCodexDone = useCallback((enabled: boolean) => {
    setAiAutoNameOnCodexDone(enabled);
    saveAiAutoNameOnCodexDone(enabled);
  }, []);
  ...
  const stepTerminalFontSize = useCallback(
    (delta: number) => {
      ...
      const fallback = Number.isFinite(base)
        ? Number(base)
        : Number.isFinite(terminalFontSize)
          ? Number(terminalFontSize)
          : (getBaseTerminalFontSize() ?? 14);
  ...
  const getLastKnownCwd = useCallback(() => {
    const fallback = preferCodeRootCwd(loadLastCwd());
    ...
    return preferCodeRootCwd(fromMeta) ?? fallback;
    ...
    async (options?: { mode?: Mode; readonlyPath?: string; tmuxName?: string; launchTerminal?: boolean }) => {
      ...
      const run = (async () => {
        try {
          ...
          const fallbackSuffix = formatSessionFallbackSuffix(attach);
          setStatusLine(`${attach.created ? "Connected" : "Reattached"} (${attach.sessionId})${fallbackSuffix}`);
  ...
  const dispatchPendingQuickLaunchCommand = useCallback(
    (sessionId: string, source: "ws_open" | "timer") => {
      const pending = pendingQuickLaunchCommandRef.current;
      if (!pending) return false;
      if (pending.sessionId !== sessionId) return false;
      const command = pending.command.replace(/[\r\n]+$/g, "");
      pendingQuickLaunchCommandRef.current = null;
      if (!command) return false;
      terminalRef.current?.sendText(`${command}\r`);
      terminalRef.current?.focus();
      setStatusLine(`${pending.label} started in tmux:${pending.tmuxName}.`);
      addClientLog("quick_launch.command_sent", {
        sessionId,
        label: pending.label,
  ...
  const handleSocketEvent = useCallback(
      ...
      if (event === "ws.open") {
        ...
        const openedSessionId =
          typeof data?.sessionId === "string"
            ? data.sessionId
            : connRef.current.status === "connected"
              ? connRef.current.sessionId
              : "";
        if (openedSessionId) {
          dispatchPendingQuickLaunchCommand(openedSessionId, "ws_open");
        }
        return;
      }
      if (event === "ws.stale_session") {
        clearReconnect("stale_session");
        safeRemove(sessionStore, SESSION_STORAGE_KEY);
        setSessionId(null);
        setStatusLine("Session expired. Reconnecting…");
        void attachOrCreate().catch(() => {
          // attachOrCreate already updates status/logs
        });
        return;
    ...
    [addClientLog, clearReconnect, dispatchPendingQuickLaunchCommand, attachOrCreate, sessionStore]
  );
  ...
  const connect = useCallback(
    async (sessionIdToConnect: string) => {
      try {
        ...
        const initialSnapshot = getInitialSnapshot();
        const preferredCwd = getLastKnownCwd();
        const fallbackTmuxName = mode === "tmux" ? loadLastTmuxName(sessionStore) : null;
        const attachBody: Record<string, unknown> = {
          ...(size ?? {}),
          resumeKey,
          mode
        };
        if (preferredCwd) attachBody.cwd = preferredCwd;
        if (fallbackTmuxName) attachBody.tmuxName = fallbackTmuxName;
        if (initialSnapshot) attachBody.initialSnapshot = initialSnapshot;
        const attach = await postJson<CreateSessionResponse>(`${apiBase}/sessions/${sessionIdToConnect}/attach`, attachBody);
        const wsUrl = new URL(attach.wsUrl, window.location.href);
        wsUrl.protocol = wsBase;
        safeSet(sessionStore, SESSION_STORAGE_KEY, attach.sessionId);
        setSessionId(attach.sessionId);
  ...
  const onCreate = useCallback(async () => {
    try {
      ...
      const fallbackSuffix = formatSessionFallbackSuffix(created);
      setStatusLine(`Connected (${created.sessionId})${fallbackSuffix}`);
  ...
  const restartConsoleStack = useCallback(
    async (target: "prod" | "root" | "dev") => {
      ...
      const label =
        target === "prod"
          ? "prod console (console.caravanflow.com)"
          : target === "root"
            ? "root console (root.caravanflow.com)"
            : "dev console + root (local)";
  ...
  const onAttachPersistent = useCallback(
    async (name: string): Promise<string | null> => {
      ...
      // Reuse an existing server-side session attached to this tmux session to avoid duplicates.
      if (existing) {
        try {
    ...
    [apiBase, activeSessions, wsBase, refreshSessions, getTerminalSize, sessionId, sessionStore, setConnectedConn, updateSessionMeta]
  );

  const openQuickTmuxTool = useCallback(
    async (target: "gemini" | "codex") => {
      const defaults =
        target === "gemini"
          ? {
              label: "Gemini" as const,
              tmuxName: DEFAULT_QUICK_GEMINI_TMUX_NAME,
              command: DEFAULT_QUICK_GEMINI_COMMAND
            }
          : {
              label: "Codex" as const,
              tmuxName: DEFAULT_QUICK_CODEX_TMUX_NAME,
              command: DEFAULT_QUICK_CODEX_COMMAND
            };

      const tmuxNameRaw = target === "gemini" ? quickGeminiTmuxName : quickCodexTmuxName;
      const commandRaw = target === "gemini" ? quickGeminiCommand : quickCodexCommand;
      const tmuxName = sanitizeTmuxNameCandidate(tmuxNameRaw) || defaults.tmuxName;
      const command = String(commandRaw ?? "").trim() || defaults.command;

      if (!isValidTmuxName(tmuxName)) {
        setStatusLine(`Invalid tmux name for ${defaults.label}.`);
        return;
      }

      pendingQuickLaunchCommandRef.current = null;
      setStatusLine(`Opening ${defaults.label}…`);

      const connectedSessionId = await onAttachPersistent(tmuxName);
      if (!connectedSessionId) return;

      pendingQuickLaunchCommandRef.current = {
        sessionId: connectedSessionId,
        command,
      ...
      window.setTimeout(() => {
        dispatchPendingQuickLaunchCommand(connectedSessionId, "timer");
      }, 160);
    },
    [
      quickGeminiTmuxName,
      ...
      quickCodexCommand,
      onAttachPersistent,
      dispatchPendingQuickLaunchCommand
    ]
  );
  ...
  const startCodexExec = useCallback(() => {
    ...
    codexSessionIdRef.current = activeSessionId;
    if (sessionKey) {
      const now = Date.now();
      const promptForMeta =
      ...
      codexPromptUsedRef.current = promptForMeta;
      codexModelUsedRef.current = modelForMeta.length <= 80 ? modelForMeta : modelForMeta.slice(0, 80);
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (msg?.type === "exit") {
          ...
          const sessionForName = codexSessionIdRef.current;
          if (key) {
          ...
          if (aiAutoNameOnCodexDone && sessionForName && key) {
            void Promise.resolve(
              suggestAiNameForSessionRef.current?.(sessionForName, key, {
                includeOutput: false,
                applyManual: true,
                codexPrompt: codexPromptUsedRef.current,
                codexLogTail: codexLogTailRef.current,
                codexModel: codexModelUsedRef.current
              })
            ).catch(() => {
  ...
  useEffect(() => {
    const wsUrl = makeWsUrl(basePath ? `${basePath}/ws/agent?agent_id=console_tower` : "/ws/agent?agent_id=console_tower");
    ...
    ws.onmessage = async (event) => {
      ...
      try {
        const msg = JSON.parse(event.data);
        if (msg?.type === "SCREENSHOT_CAPTURED" && msg.payload?.dataUrl) {
          // 1. Trigger visual feedback (flash)
          setShowScreenshotFlash(true);
          setTimeout(() => setShowScreenshotFlash(false), 600);

          // 2. Convert dataUrl to File and upload
          const dataUrl = msg.payload.dataUrl;
          const res = await fetch(dataUrl);
          const blob = await res.blob();
          const file = new File([blob], `screenshot-${Date.now()}.png`, { type: "image/png" });
          
          void addCodexImage(file, "paste");
        }
      } catch (err) {
        console.error("[agent-ws] Failed to process message:", err);
      }
    };
  ...
  }, [basePath, makeWsUrl, addCodexImage]);
    ...
    async (
      ...
      const run = (async () => {
        try {
          ...
          if (options?.applyManual && meta?.nameSource === "user") {
            addClientLog("ai_name.suggest.skip_user", { sessionId: targetSessionId });
            return;
          }
          const codexPrompt = String(options?.codexPrompt ?? meta?.codexLastPrompt ?? "").trim();
          if (codexPrompt) payload.codexPrompt = codexPrompt;
          const codexLogTail = String(options?.codexLogTail ?? meta?.codexLogTail ?? "").trim();
          if (codexLogTail) payload.codexLogTail = codexLogTail;
          const codexModelLabel = String(options?.codexModel ?? meta?.codexModel ?? "").trim();
  ...
  // Keep a stable handle for earlier callbacks (Codex exec handlers) without pulling this into their deps.
  suggestAiNameForSessionRef.current = suggestAiNameForSession;
  ...
  const [snowMenuOpen, setSnowMenuOpen] = useState(false);
  const snowMenuRef = useRef<HTMLDivElement | null>(null);
  const snowMenuButtonRef = useRef<HTMLButtonElement | null>(null);

  const openSnowExternalUrl = useCallback((url: string) => {
    if (typeof window === "undefined") return;
  ...
  const triggerSnowMenuAction = useCallback(
    (action: SnowMenuActionId) => {
      setSnowMenuOpen(false);
      switch (action) {
        case "newSession":
          onCreate();
          return;
        case "openGemini":
          void openQuickTmuxTool("gemini");
          return;
        case "openCodex":
          void openQuickTmuxTool("codex");
          return;
        case "disconnect":
          onDisconnectOnly();
          return;
        ...
        case "codexBrief":
          ...
          return;
        case "openAllTmux":
          void openAllTmuxSessions();
          return;
        case "refreshTab":
          void onRefreshCurrentTab();
          return;
        case "docs":
          openSnowExternalUrl(docsUrl);
          return;
        case "tasks":
          openSnowExternalUrl(tasksUrl);
          return;
      }
    },
    [
  ...
  const renderSnowMenuItemText = useCallback(
    (action: SnowMenuActionId, label: string) => (
      <span className="snowMenuItemContent">
        <span className="snowMenuItemIcon" aria-hidden="true" style={maskStyleForSvgButton(SN_MENU_ICON_NAMES[action])} />
        <span className="snowMenuItemLabel">{label}</span>
        <span className="snowMenuItemHint" aria-hidden="true">
          {SN_SHORTCUT_HINTS[action]}
        </span>
      </span>
    ),
    [maskStyleForSvgButton]
  );

  useEffect(() => {
    if (!snowMenuOpen) return;
    const onPointerDown = (event: Event) => {
      const target = event.target;
      if (!target || !(target instanceof Node)) return;
      if (snowMenuRef.current?.contains(target)) return;
      if (snowMenuButtonRef.current?.contains(target)) return;
      setSnowMenuOpen(false);
    };
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const hasPrimaryShortcut = (event.metaKey || event.ctrlKey) && event.altKey && !event.shiftKey;
      const isEditableTarget = isEditableEventTarget(event.target);

      if (!hasPrimaryShortcut && snowMenuOpen && !event.metaKey && !event.ctrlKey && !event.altKey) {
        const action = SN_MENU_KEYMAP[key];
        if (!action) return;
        if (isEditableTarget) return;
        event.preventDefault();
        event.stopPropagation();
        triggerSnowMenuAction(action);
        return;
      }

      if (!hasPrimaryShortcut) return;
      if (isEditableTarget && !snowMenuOpen) return;

      const mapped = SN_GLOBAL_KEYMAP[event.code];
      if (!mapped) return;

      event.preventDefault();
      event.stopPropagation();

      if (mapped === "toggleMenu") {
        setSnowMenuOpen((prev) => !prev);
        return;
      }

      triggerSnowMenuAction(mapped);
    };
  ...
  }, [snowMenuOpen, triggerSnowMenuAction]);
  ...
  return (
    <div
      className={`app servicenowTheme ${controlsHidden ? "compact" : ""} ${controlsHidden ? `compactDock-${compactDockEdge}` : ""} ${
        settingsOpen ? "settingsOpen" : ""
      } ${showTabletHelperBar ? "showTerminalMobileBar" : ""}`}
    >
      <header className="snowHeader" data-testid="snow-header">
        <div className="snowHeaderTop">
          <div className="snowBrand" aria-label="ServiceNow">
            <span className="snowBrandService">service</span>
            <span className="snowBrandNow">
              <span>n</span>
              <span className="snowBrandO">
                o
                <span className="snowBrandDot" aria-hidden="true" />
              </span>
              <span>w</span>
            </span>
            <span className="snowBrandReg">®</span>
          </div>
          <div className="snowTopActions">
            <button
              className="snowIconBtn"
              type="button"
              aria-label="Search"
              title="Search"
              onClick={() => setStatusLine("Search is not wired in this console clone yet.")}
            >
              <svg viewBox="0 0 24 24" aria-hidden="true">
                <path d="M11 4a7 7 0 1 0 4.9 12l4 4a1 1 0 0 0 1.4-1.4l-4-4A7 7 0 0 0 11 4Zm0 2a5 5 0 1 1 0 10 5 5 0 0 1 0-10Z" />
              </svg>
            </button>
            <button
              className="snowIconBtn"
              ...
              aria-label="Language"
              title="Language"
              onClick={() => setStatusLine("Language menu is not wired in this console clone yet.")}
            >
              <svg viewBox="0 0 24 24" aria-hidden="true">
                <path d="M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18Zm6.9 8h-3.1a14.4 14.4 0 0 0-1-4.3A7 7 0 0 1 18.9 11ZM12 5c.7 0 2 1.9 2.4 6H9.6C10 6.9 11.3 5 12 5Zm-3.8 1.7A14.4 14.4 0 0 0 7.2 11H4.1a7 7 0 0 1 4.1-4.3ZM4.1 13h3.1c.2 1.6.6 3.1 1 4.3A7 7 0 0 1 4.1 13Zm7.9 6c-.7 0-2-1.9-2.4-6h4.8c-.4 4.1-1.7 6-2.4 6Zm2.8-1.7c.4-1.2.8-2.7 1-4.3h3.1a7 7 0 0 1-4.1 4.3Z" />
              </svg>
            </button>
            <button
              className="snowProfileBtn"
              type="button"
              aria-label="Profile"
              title="Profile"
              onClick={() => setStatusLine("Profile menu is not wired in this console clone yet.")}
            >
              R
            </button>
            <div className="snowMenuWrap">
              <button
                ref={snowMenuButtonRef}
                className="snowIconBtn snowHamburgerBtn"
                type="button"
                data-testid="snow-hamburger"
                aria-haspopup="menu"
                aria-expanded={snowMenuOpen}
                aria-controls="snow-controls-menu"
                aria-label="Main menu"
                title="Main menu"
                onClick={() => setSnowMenuOpen((prev) => !prev)}
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
              ...
              {snowMenuOpen && (
                <div
                  ref={snowMenuRef}
                  id="snow-controls-menu"
                  className="snowMenuDropdown"
                  role="menu"
                  data-testid="snow-menu"
                >
                  <div className="snowMenuSectionLabel">Console Controls</div>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("newSession")}
                    aria-keyshortcuts="Control+Alt+N Meta+Alt+N"
                    disabled={conn.status === "connecting" || creatingSession}
                  >
                    {renderSnowMenuItemText("newSession", "New session")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("openGemini")}
                    aria-keyshortcuts="Control+Alt+G Meta+Alt+G"
                    disabled={conn.status === "connecting" || creatingSession}
                  >
                    {renderSnowMenuItemText("openGemini", "Open Gemini")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("openCodex")}
                    aria-keyshortcuts="Control+Alt+C Meta+Alt+C"
                    disabled={conn.status === "connecting" || creatingSession}
                  >
                    {renderSnowMenuItemText("openCodex", "Open Codex")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("disconnect")}
                    aria-keyshortcuts="Control+Alt+D Meta+Alt+D"
                    disabled={conn.status !== "connected"}
                  >
                    {renderSnowMenuItemText("disconnect", "Disconnect")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("reconnect")}
                    aria-keyshortcuts="Control+Alt+R Meta+Alt+R"
                    disabled={!sessionId && !safeGet(sessionStore, SESSION_STORAGE_KEY)}
                  >
                    {renderSnowMenuItemText("reconnect", "Reconnect")}
                  </button>
                  <button
                    className="snowMenuItem snowMenuDanger"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("deleteSession")}
                    aria-keyshortcuts="Control+Alt+X Meta+Alt+X"
                    disabled={!sessionId && !safeGet(sessionStore, SESSION_STORAGE_KEY)}
                  >
                    {renderSnowMenuItemText("deleteSession", "Delete session")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("sessions")}
                    aria-keyshortcuts="Control+Alt+S Meta+Alt+S"
                  >
                    {renderSnowMenuItemText("sessions", sessionsOpen ? "Close sessions" : "Sessions")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("settings")}
                    aria-keyshortcuts="Control+Alt+E Meta+Alt+E"
                  >
                    {renderSnowMenuItemText("settings", settingsOpen ? "Close settings" : "Settings")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("aiRename")}
                    aria-keyshortcuts="Control+Alt+A Meta+Alt+A"
                    disabled={aiRenameDisabled}
                  >
                    {renderSnowMenuItemText("aiRename", "AI rename open sessions")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("codexBrief")}
                    aria-keyshortcuts="Control+Alt+B Meta+Alt+B"
                    disabled={briefRunning}
                  >
                    {renderSnowMenuItemText("codexBrief", briefRunning ? "Generating brief..." : "Codex brief")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("openAllTmux")}
                    aria-keyshortcuts="Control+Alt+O Meta+Alt+O"
                    disabled={openingAllTmuxSessions || availableTmuxSessions.sessions.length === 0}
                  >
                    {renderSnowMenuItemText("openAllTmux", "Open all tmux sessions")}
                  </button>
                  <button
                    className="snowMenuItem"
                    role="menuitem"
                    onClick={() => triggerSnowMenuAction("refreshTab")}
                    aria-keyshortcuts="Control+Alt+F Meta+Alt+F"
                  >
                    {renderSnowMenuItemText("refreshTab", "Refresh tab")}
                  </button>
                  <a
                    className="snowMenuItem"
                    role="menuitem"
                    ...
                    target="_blank"
                    rel="noreferrer"
                    onClick={() => setSnowMenuOpen(false)}
                    aria-keyshortcuts="Control+Alt+H Meta+Alt+H"
                  >
                    {renderSnowMenuItemText("docs", "Docs")}
                  </a>
                  <a
                    className="snowMenuItem"
                    role="menuitem"
                    ...
                    rel="noreferrer"
                    onClick={() => setSnowMenuOpen(false)}
                    aria-keyshortcuts="Control+Alt+L Meta+Alt+L"
                  >
                    {renderSnowMenuItemText("tasks", "Task library")}
                  </a>
                </div>
              )}
            </div>
        ...
        <div className="snowHeaderBottom">
          <span className="snowHeaderBottomTitle">Developer</span>
          <button
            className="snowChevronBtn"
            type="button"
      ...
      {isSmallScreen && (
                      ...
                      <div className="studioBadge">SN DEVELOPER</div>
        	          </div>
                ...
                case "sessions":
                  button = (
                    <button
                      ...
                      aria-label={
                        sessionsOpen ? "Close sessions" : attentionLabel ? `Sessions (${attentionLabel})` : "Sessions"
                      }
                      title={sessionsOpen ? "Close sessions" : attentionLabel ? `Sessions (${attentionLabel})` : "Sessions"}
                    >
                      <span
                ...
                case "aiRename":
                  button = (
                    <button
                      ...
                      disabled={aiRenameDisabled}
                      aria-label="Auto-name open sessions"
                      title={aiRenameTitle}
                    >
      ...
      {showFullControls && (
        ...
        <div className="studioBadge">SN DEVELOPER</div>
        <select className="select" value={mode} onChange={(e) => setMode(e.target.value as Mode)} disabled>
        ...
        <button
          className="button"
          onClick={() => void openQuickTmuxTool("gemini")}
          disabled={conn.status === "connecting" || creatingSession}
          aria-label="Open Gemini tmux session"
          title={`Open tmux:${quickGeminiTmuxName || DEFAULT_QUICK_GEMINI_TMUX_NAME} and run Gemini command`}
        >
          <span className="buttonLabel">Open Gemini</span>
        </button>
        <button
          className="button"
          onClick={() => void openQuickTmuxTool("codex")}
          disabled={conn.status === "connecting" || creatingSession}
          aria-label="Open Codex tmux session"
          title={`Open tmux:${quickCodexTmuxName || DEFAULT_QUICK_CODEX_TMUX_NAME} and run Codex command`}
        >
          <span className="buttonLabel">Open Codex</span>
        </button>
        <button
          className="button"
          onClick={onDisconnectOnly}
            ...
            >
              ...
              <div className="settingsBody">
                <div className="audioPanel terminalPanel">
                  ...
                  <div className="audioSection">
                    ...
                    <div className="audioRow">
                      <label
                        className="audioToggle"
                        title="When enabled, terminal-specific mouse/keyboard handlers capture wheel gestures and shortcuts. Turn it off to let the page (F12, Fn keys, scrolling) behave normally."
                      >
                        <input
                          type="checkbox"
                          checked={terminalEventOverridesEnabled}
                          onChange={(e) => applyTerminalEventOverridesEnabled(e.target.checked)}
                        />
                        Capture terminal mouse/keyboard events
                      </label>
                    </div>
                    {!terminalEventOverridesEnabled && (
                      <div className="audioNotice">
                        Terminal gestures, page scrolling, and browser shortcuts are handed back to the document.
                      </div>
                    )}
                  </div>
                  <div className="audioSection">
                    <div className="audioSectionTitle">Quick launch</div>
                    <div className="audioRow">
                      <input
                        className="audioInput"
                        value={quickGeminiTmuxName}
                        onChange={(e) => applyQuickGeminiTmuxName(e.target.value)}
                        placeholder="Gemini tmux name"
                        aria-label="Gemini tmux session name"
                      />
                      <input
                        className="audioInput"
                        value={quickGeminiCommand}
                        onChange={(e) => applyQuickGeminiCommand(e.target.value)}
                        placeholder="Gemini command"
                        aria-label="Gemini startup command"
                      />
                      <button
                        type="button"
                        className="button"
                        onClick={() => void openQuickTmuxTool("gemini")}
                        disabled={conn.status === "connecting" || creatingSession}
                        title="Open tmux session and run Gemini command"
                      >
                        Open Gemini
                      </button>
                    </div>
                    <div className="audioRow">
                      <input
                        className="audioInput"
                        value={quickCodexTmuxName}
                        onChange={(e) => applyQuickCodexTmuxName(e.target.value)}
                        placeholder="Codex tmux name"
                        aria-label="Codex tmux session name"
                      />
                      <input
                        className="audioInput"
                        value={quickCodexCommand}
                        onChange={(e) => applyQuickCodexCommand(e.target.value)}
                        placeholder="Codex command"
                        aria-label="Codex startup command"
                      />
                      <button
                        type="button"
                        className="button"
                        onClick={() => void openQuickTmuxTool("codex")}
                        disabled={conn.status === "connecting" || creatingSession}
                        title="Open tmux session and run Codex command"
                      >
                        Open Codex
                      </button>
                    </div>
                ...
                <div className="audioPanel">
                  <div className="audioHeader">
                    <div className="audioTitle">System</div>
                  </div>
                  <div className="audioSection">
                    <div className="audioRow">
                      <a href="/ghost" className="button" style={{ textDecoration: 'none', width: '100%', textAlign: 'center' }}>
                        Switch to Ghost Console (Lightweight)
                      </a>
                    </div>
                  </div>
                </div>
	                  ...
	                  <div className="audioSection">
	                    ...
	                    <div className="audioRow">
	                      ...
	                      >
		                      ...
		                      <label
		                        className="audioToggle"
		                        title="After Codex exec exits, apply an AI-generated task name to the originating session tab. Never overwrites user-set names."
		                        onClick={(e) => {
		                          if ((e.target as HTMLElement).tagName === "INPUT") return;
		                          if (!aiStatusLoaded || !aiNamingEnabled) return;
		                          e.preventDefault();
		                          applyAiAutoNameOnCodexDone(!aiAutoNameOnCodexDone);
		                        }}
		                      >
		                        <input
		                          type="checkbox"
		                          checked={aiAutoNameOnCodexDone}
		                          disabled={!aiStatusLoaded || !aiNamingEnabled}
		                          data-testid="ai-autoname-codex-done"
		                          onChange={(e) => applyAiAutoNameOnCodexDone(e.target.checked)}
		                        />
		                        Auto-rename on Codex done
		                      </label>
		                      <label
		                        className="audioToggle"
        ...
        <div className={`terminalWrap${codexActive ? " codexRunning" : ""}`} ref={terminalWrapRef}>
          ...
          <TerminalView
            ...
            eventOverridesEnabled={terminalEventOverridesEnabled}
          />
      ...
      {showScreenshotFlash && (
        <div 
          ...
          style={{
            ...
            zIndex: 9999,
            pointerEvents: 'none',
            animation: 'screenshot-flash-fade 0.6s ease-out forwards'
          }}
        />
      )}
    </div>
