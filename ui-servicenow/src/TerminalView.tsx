...
export type TerminalViewHandle = {
  ...
  getScrollState: () => { viewportY: number; baseY: number; isAtBottom: boolean; inAlternateScreen: boolean; mouseMode: boolean } | null;
...
const FIT_RESIZE_DEBOUNCE_MS = 180;
const FIT_KEYBOARD_DEBOUNCE_MS = 90;
const SNAPSHOT_FINGERPRINT_EDGE = 96;
const SCROLL_STATE_TRAIL_MS = 48;
const TRACKPAD_MIN_DELTA_PX = 0.01;

const clampFontSize = (size: number): number =>
  Math.min(FONT_SIZE_MAX, Math.max(FONT_SIZE_MIN, Math.round(size)));

const snapshotFingerprint = (text: string): string => {
  if (!text) return "0::";
  if (text.length <= SNAPSHOT_FINGERPRINT_EDGE * 2) {
    return `${text.length}:${text}:`;
  }
  return `${text.length}:${text.slice(0, SNAPSHOT_FINGERPRINT_EDGE)}:${text.slice(-SNAPSHOT_FINGERPRINT_EDGE)}`;
};

const resolveTerminalViewportElement = (term: Terminal | null): HTMLDivElement | null => {
  const element = term?.element;
  if (!element) return null;
  return (
    (element.querySelector(".xterm-viewport") as HTMLDivElement | null) ||
    (element.querySelector(".xterm-screen") as HTMLDivElement | null) ||
    (element as HTMLDivElement)
  );
};

const isAlternateScreenActive = (term: any): boolean => {
  const buffers = term?.buffers;
  if (buffers?.active && buffers?.alternate) {
    return buffers.active === buffers.alternate;
  }
  const bufferSet = term?._core?._bufferService?.bufferSet;
  if (bufferSet?.active && bufferSet?.alt) {
    return bufferSet.active === bufferSet.alt;
  }
  return false;
};

const isMouseModeActive = (term: any): boolean => {
  const element = term?.element as HTMLElement | undefined;
  if (element?.classList?.contains("enable-mouse-events")) return true;
  const dec = term?._core?._coreService?.decPrivateModes;
  if (!dec || typeof dec !== "object") return false;
  const mode = dec.mouseTrackingMode;
...
export const TerminalView = forwardRef<TerminalViewHandle, Props>(
  ...
  const scrollCheckTrailRef = useRef<number | null>(null);
  const [isAtBottom, setIsAtBottom] = useState(true);
  const atBottomRef = useRef(true);
  const activeSessionIdRef = useRef<string | null>(null);
  const sessionViewportByIdRef = useRef<Map<string, number>>(new Map());
  ...
  const wsWriteQueueRef = useRef<string[]>([]);
  const wsWriteFrameRef = useRef<number | null>(null);
  const wsRestoreViewportYRef = useRef<number | null>(null);
  const lastSnapshotFingerprintRef = useRef<string | null>(null);
  ...
  const scheduleScrollCheck = useCallback(() => {
    ...
    if (scrollCheckTrailRef.current !== null) {
      window.clearTimeout(scrollCheckTrailRef.current);
    }
    scrollCheckTrailRef.current = window.setTimeout(() => {
      scrollCheckTrailRef.current = null;
      if (scrollCheckRef.current !== null) return;
      scrollCheckRef.current = window.requestAnimationFrame(() => {
        scrollCheckRef.current = null;
        updateAtBottom();
      });
    }, SCROLL_STATE_TRAIL_MS);
  }, [updateAtBottom]);

  const cancelQueuedWrites = useCallback(() => {
    if (wsWriteFrameRef.current !== null) {
      window.cancelAnimationFrame(wsWriteFrameRef.current);
      wsWriteFrameRef.current = null;
    }
    wsWriteQueueRef.current = [];
    wsRestoreViewportYRef.current = null;
  }, []);
  ...
  const flushQueuedWrites = useCallback(() => {
    ...
    const chunks = wsWriteQueueRef.current;
    if (!chunks.length) return;
    wsWriteQueueRef.current = [];
    const restoreViewportY = wsRestoreViewportYRef.current;
    wsRestoreViewportYRef.current = null;
    term.write(chunks.join(""), () => {
      if (restoreViewportY !== null && term.buffer.active.baseY > 0) {
        const maxViewportY = term.buffer.active.baseY;
        term.scrollToLine(Math.min(restoreViewportY, maxViewportY));
      }
      const activeSessionId = activeSessionIdRef.current;
      if (activeSessionId) {
        const savedSessionViewportRaw = sessionViewportByIdRef.current.get(activeSessionId);
        if (typeof savedSessionViewportRaw === "number" && Number.isFinite(savedSessionViewportRaw)) {
          const savedSessionViewport = savedSessionViewportRaw;
          const maxViewportY = Number(term?.buffer?.active?.baseY ?? 0);
          if (savedSessionViewport > 0 && maxViewportY > 0) {
            const targetViewportY = Math.max(0, Math.min(savedSessionViewport, maxViewportY));
            term.scrollToLine(targetViewportY);
            atBottomRef.current = targetViewportY >= maxViewportY;
  ...
  const queueTerminalWrite = useCallback(
    (text: string) => {
      ...
      const term = termRef.current;
      if (!term) return;
      wsWriteQueueRef.current.push(text);
      if (!atBottomRef.current && wsRestoreViewportYRef.current === null) {
        wsRestoreViewportYRef.current = term.buffer.active.viewportY;
      }
      if (wsWriteFrameRef.current !== null) return;
      wsWriteFrameRef.current = window.requestAnimationFrame(flushQueuedWrites);
    },
    [flushQueuedWrites]
  );
      ...
      const run = () => {
        ...
        fitFrameRef.current = window.requestAnimationFrame(() => {
          ...
          try {
            ...
            // ignore fit errors while layout is in transition
          }
          scheduleScrollCheck();
        });
      };
  ...
  useEffect(() => {
    ...
    const updateViewport = () => {
      ...
      scheduleFit(FIT_KEYBOARD_DEBOUNCE_MS);
      scheduleScrollCheck();
    };
  ...
  useEffect(() => {
    ...
    const viewport = resolveTerminalViewportElement(terminal);
    ...
    const onScroll = terminal.onScroll(() => {
      ...
      if (sessionId) {
        ...
        if (Number.isFinite(viewportY)) {
          sessionViewportByIdRef.current.set(sessionId, viewportY);
        }
      }
    ...
    const applyIOSWritingAssistHack = () => {
      ...
      // Keep the textarea at >=16px to prevent Safari auto-zoom on focus.
    ...
    const applyFit = (delayMs = FIT_RESIZE_DEBOUNCE_MS) => {
    ...
    const onWindowResize = () => applyFit(FIT_RESIZE_DEBOUNCE_MS);
  ...
  }, [scheduleFit, scheduleScrollCheck, initialIsSmall]);

  const overridesEnabled = eventOverridesEnabled ?? true;
  const wheelRedirectEnabled = getIsAppleTouchDevice();
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (event: TouchEvent) => {
      ...
      const state = touchScrollRef.current;
      if (event.touches.length !== 1) return;
      if (!swipe.active && !state.active) return;

      const touch = event.touches[0];
      const dx = touch.clientX - swipe.startX;
      const dy = touch.clientY - swipe.startY;
      if (state.active && (Math.abs(dy) > 3 || Math.abs(dx) > 3)) {
        state.moved = true;
      }
    ...
    const handleWheelGlobalCapture = (event: WheelEvent) => {
      if (!wheelRedirectEnabled) return;
      ...
      const term = termRef.current;
      const target = resolveTerminalViewportElement(term) ?? (term?.element as HTMLElement | null);
      if (!target) return;
    ...
    // Route wheel behavior:
    // Shift = force scrollback, Alt = force app, default = scrollback when available.
    // This keeps Codex/tmux sessions reliably scrollable; hold Alt to send wheel to the app.
    const handleWheelCapture = (event: WheelEvent) => {
      ...
      const viewport = resolveTerminalViewportElement(term) ?? viewportRef.current;
      if (!term || !viewport) return;
      const baseY = Number(term?.buffer?.active?.baseY ?? 0);
      const hasScrollback = Number.isFinite(baseY) && baseY > 0;
      const forceScrollback = event.shiftKey;
      const forceApp = event.altKey;
      const shouldHandleScrollback = forceScrollback || (!forceApp && hasScrollback);
      if (!shouldHandleScrollback) return;
      ...
      if (event.deltaMode === 0 && Math.abs(event.deltaY) < TRACKPAD_MIN_DELTA_PX) return;
      const deltaPx =
        event.deltaMode === 1
  ...
  }, [applyFontSize, focusTerminal, moveCursorToPoint, onSwipe, scheduleScrollCheck, overridesEnabled, wheelRedirectEnabled]);
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      scrollByLines: (lines: number) => {
        ...
        termRef.current?.scrollLines(Math.trunc(lines));
        scheduleScrollCheck();
      },
      getScrollState: () => {
        const term = termRef.current as any;
        ...
        return {
          viewportY: Number.isFinite(viewportY) ? viewportY : 0,
          baseY: Number.isFinite(baseY) ? baseY : 0,
          isAtBottom: baseY === viewportY,
          inAlternateScreen: isAlternateScreenActive(term),
          mouseMode: isMouseModeActive(term)
        };
      },
      getSize: () => {
        const term = termRef.current;
    ...
    [sendInput, focusTerminal, scheduleFit, scheduleScrollCheck, applyFontSize]
  );
  ...
  useEffect(() => {
    ...
    const saveSessionViewport = (sessionId: string | null | undefined) => {
      if (!sessionId) return;
      const viewportY = Number(terminal?.buffer?.active?.viewportY ?? 0);
      if (Number.isFinite(viewportY)) {
        sessionViewportByIdRef.current.set(sessionId, viewportY);
    ...
    const restoreSessionViewport = (sessionId: string | null | undefined) => {
      if (!sessionId) return false;
      const savedRaw = sessionViewportByIdRef.current.get(sessionId);
      if (typeof savedRaw !== "number" || !Number.isFinite(savedRaw)) return false;
      const saved = savedRaw;
      const maxViewportY = Number(terminal?.buffer?.active?.baseY ?? 0);
      if (saved > 0 && maxViewportY <= 0) return false;
      const targetViewportY = Math.max(0, Math.min(saved, Math.max(0, maxViewportY)));
    ...
    if (previousSessionId && (conn.status !== "connected" || previousSessionId !== conn.sessionId)) {
      saveSessionViewport(previousSessionId);
    }
    activeSessionIdRef.current = conn.status === "connected" ? conn.sessionId : null;
    ...
    const connectTimer = window.setTimeout(() => {
      ...
      ws.onopen = () => {
        ...
        focusTerminal();
        scheduleFit(0);
        if (!restoreSessionViewport(conn.sessionId)) {
          termRef.current?.scrollToBottom();
          atBottomRef.current = true;
          setIsAtBottom(true);
        }
        ...
        if (typeof event.data === "string") {
	          try {
	            ...
	            if (msg?.type === "snapshot" && typeof msg.data === "string") {
	              ...
	              if (!term) return;
                cancelQueuedWrites();
                const fingerprint = snapshotFingerprint(msg.data);
                if (lastSnapshotFingerprintRef.current === fingerprint) {
	            ...
	            if (msg?.type === "stale_session") {
	              emitSocketEvent("ws.stale_session", {
	                sessionId: conn.sessionId,
	                attempt
	              });
	            }
    ...
    return () => {
      ...
      if (activeSessionIdRef.current) {
        const viewportY = Number(terminal?.buffer?.active?.viewportY ?? 0);
        if (Number.isFinite(viewportY)) {
          sessionViewportByIdRef.current.set(activeSessionIdRef.current, viewportY);
        }
      }
      cancelQueuedWrites();
      ...
      <div className="terminalMobileBar" onClick={(event) => event.stopPropagation()}>
        ...
        >
          <span
            className="terminalMobileIcon"
            aria-hidden="true"
            style={maskStyleForSvgButton("ui-arrow-up.svg")}
          />
        </button>
        <button
          type="button"
