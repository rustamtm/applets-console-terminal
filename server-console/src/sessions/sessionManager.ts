...
export type SessionManagerConfig = {
  ...
  onExit?: (sessionId: string, exitCode: number | null, signal: number | null) => void;
...
export class SessionManager {
  ...
  create(user: AccessUser, req: CreateSessionRequest): Session {
    ...
    // Chat UI is not a full terminal emulator, but some interactive CLIs query the terminal
    // for cursor position/device attributes (DSR/DA). The headless xterm instance can
    // generate those replies via `onData`. Only forward them to the PTY when there are
    // no active/pending terminal (xterm) web connections to avoid duplicate responses.
    headless.onData((reply) => {
      if (session.closed) return;
    ...
    pty.onExit(({ exitCode, signal }) => {
      ...
      session.closed = true;
      try {
        this.cfg.onExit?.(session.id, exitCode ?? null, signal ?? null);
      } catch {
