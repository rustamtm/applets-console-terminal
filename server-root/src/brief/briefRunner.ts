...
export async function runCodexBrief(opts: RunOptions): Promise<BriefRunResult> {
  ...
  const system =
    "You are a status summarizer for my local dev context. " +
    "Produce concise, actionable status, no speculation. " +
    "Use only the provided tmux/task signals. If uncertain, say so.";
  const user =
    "Given tmux context and recent tasks, tell me what is done, what is running, what is likely in progress, and what I should do next. " +
    "Return JSON matching the provided schema exactly.\n\n" +
    `Context JSON:\n${sanitizeForPrompt(JSON.stringify(contextPayload), redaction, prefs.redactPaths)}`;
